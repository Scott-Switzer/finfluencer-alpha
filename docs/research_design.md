...
Transcript availability must be reported in the final paper. Each attempted video should record provider name, package version, language, whether the transcript is generated, retrieval status, error type, segment count, and full-text hash. If the provider reports request blocking, IP blocking, or rate limiting, the pipeline records the status and treats the video as missing data. Package-supported proxy configuration may be used only as an explicitly configured transport option for the free transcript package, with credentials redacted from logs and outputs. The pipeline must not use rotating-proxy escalation, residential-proxy workarounds, cookie authentication, browser automation, `yt-dlp`, audio downloading, Whisper, hidden endpoints, or access-control bypasses.

Free-package collection methods should be benchmarked before promotion. The benchmark workflow compares per-video API creation, a shared in-run API session, and the package CLI batch surface without writing transcripts to the database. Promotion requires faster measured throughput, zero block/proxy failures, and metadata that remains sufficient for the database schema.

Transcript coverage and research readiness are first-class audit outputs. Availability audits must report coverage by year and creator from local database state only, while the research-readiness report should separate defensible descriptive claims from unsupported representative or causal claims.
